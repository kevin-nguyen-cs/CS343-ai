...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        "*** YOUR CODE HERE ***"
        #Check if state exists, if not then return 0.0
    ...
    def computeActionFromQValues(self, state):
        ...
        #Local Declarations
        actionList = self.getLegalActions(state)
        #Check if legal action is empty or not
        if not actionList:
          return None
        #Loop through each action in the action list
        for action in actionList:
          #if current Q-val is highest q-val, return that action 
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.values[(state, action)] = (1 - self.alpha) * oldVal + self.alpha * sample
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        #Local Declarations
        qValue = 0.0
        myWeight = self.getWeights()
        #check if state, action pair exists already in the weight vector
        if not self.weights[(state,action)]:
         self.weights[(state,action)] = 0.0
        #Loop to go through the feature vector do DotProduct with the weights and keep track of summation
        for key in self.featExtractor.getFeatures(state, action):
          qValue += myWeight[key] * self.featExtractor.getFeatures(state, action)[key]
        return qValue #return the qValue
        util.raiseNotDefined()
