...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #loop through positions, dictionary key = position called p
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            #if there is some belief and pacman has not eaten a ghost, then update observation
            if ((emissionModel[trueDistance] > 0) and not (noisyDistance == None)):
                allPossible[p] = self.beliefs[p] * emissionModel[trueDistance]
        #if noisyDistance is none then update all beliefs to 0 and provide the jail position in the dictionary
        if (noisyDistance == None):
            count = 0
            #loop through all legal positions
            while (count < len(self.legalPositions)):
                p2 = self.legalPositions[count]
                #if not pacman position (same position he was arresting the ghost) then set belief to 0
                if not (pacmanPosition == p2):
                    allPossible[p2] = 0.0 #set belief to be 0.0
                else:
                    allPossible[self.getJailPosition()] = 1.0 #set jail belief to be 1.0
                count = count + 1 #increment loop
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            #get position distribution of the ghosts
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            #loop through new positions of the ghost and compute for the update
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += self.beliefs[oldPos] * prob #keep track of the summation of product of factors
        allPossible.normalize() #normalize the beliefs
        self.beliefs = allPossible #set temp table to the actual belief table
        #util.raiseNotDefined()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        gridLength = len(self.legalPositions)
        weight = 1.0
        count = 0
        count2 = 0
        #loop through all the positions for the particles
        while (count < gridLength):
            #loop to insert the correct number of particles per position evenly
            while count2 < (numOfParticles/gridLength):
                particle = (self.legalPositions[count]) #particle
                self.particleList.append(particle) #append particle to list
                count2 += 1 #increment counter2
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        #Local Declarations
        parentBeliefs = self.getBeliefDistribution() #Parent distributions
        currentBeliefs = util.Counter() #temp list of beliefs (weights)
        #process cases whether or not noisydistance is None
        if not (noisyDistance == None):
            ...
            for p in self.legalPositions:
                ...
                if ((emissionModel[trueDistance] > 0)):
                    currentBeliefs[p] = parentBeliefs[p] * emissionModel[trueDistance] #IS THIS CORRECT???
            currentBeliefs.normalize()
            self.handleCaseTwo(currentBeliefs, gameState)
        self.handleCaseOne(noisyDistance, currentBeliefs, pacmanPosition) 

    def handleCaseOne(self, noisyDistance, currentBeliefs, pacmanPosition):
        # #if noisyDistance is none then update all beliefs to 0 and provide the jail position in the dictionary
        if (noisyDistance == None):
            count = 0
            #loop through all particles
            self.particleList = []; #set particle list to be empty and then fill it up with jail position
            ...
            self.legalPositions.append(self.getJailPosition())
            #print "particleList: ", self.legalPositions

    def handleCaseTwo(self, currentBeliefs, gameState):
        if all(weight == 0.0 for weight in currentBeliefs.values()):
            self.initializeUniformly(gameState)
        else:
            count = 0
            tempList = []
            #loop to go through each particle
            while (count < self.numParticles):
                newSample = util.sample(currentBeliefs) #get new sample
                tempList.append(newSample) #append new sample to list
                count += 1 #increment loop
            self.particleList = tempList #set list of new samples to be new particleList
    ...
    def elapseTime(self, gameState):
        ...
        count = 0
        tempList = list()
        lengthPList = len(self.particleList)
        #Loop through all particles in list
        while (count < lengthPList):
            oldPos = self.particleList[count] #old position reference from particleList
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            newSample = util.sample(newPosDist) #get new sample
            tempList.append(newSample) #append new sample to temp
            count += 1 #increment loop
        self.particleList = tempList #set particleList to be the tempList
        #util.raiseNotDefined()
    ...
    def getBeliefDistribution(self):
        ...
        #loop through all legal positions for particles, and use pos as reference
        while (count < len(self.legalPositions)):
            positionRef = self.legalPositions[count]
            #Loop through each particle with position reference, get sum of all particles by position perspective and put into dictionary
            while (count2 < len(self.particleList)):
                particle = self.particleList[count2] #get particle from list, particle is  tuple of ((x,y), weight)
                #only add sums if position Reference matches the position of the particle
                if (particle == positionRef):
                    sumVal += 1.0
                count2 += 1 #increment counter2
            beliefList[positionRef] = sumVal #beliefList[key=pos] = value=summation of OCCURRANCES
            count += 1 #increment counter
            count2 = 0 #reset counter2
            sumVal = 0.0 #reset sumVal for next position
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particleList = list()
        self.posList = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        numOfParticles = self.numParticles
        shuffle(self.posList)
        gridLength = len(self.posList)
        weight = 1.0
        count = 0
        count2 = 0
        #self.particleList = self.posList

        while (count < numOfParticles):
            #loop to insert the correct number of particles per position evenly
            while count2 < gridLength and count < numOfParticles:
                particle = (self.posList[count2]) #particle
                self.particleList.append(particle) #append particle to list
                count2 += 1 #increment counter2
                count += 1 #increment counter
            count2 = 0 #reset counter2
    ...
    def observeState(self, gameState):
        ...
        while (count < len(self.particleList)):
            ...
            currentBeliefs[particle] += temp #update currentBeliefs with the weights
            total += temp
            count += 1 #increment loop
        ...
        if(total == 0):
            ...
            count = 0
            while (count < len(self.particleList)):
                self.particleList[count] = util.sample(currentBeliefs) #set particle to jail position and return it
                count += 1 #increment loop
        ...
        for i in range(self.numGhosts):
            if (noisyDistances[i] == None):
                ...
                while (count < len(self.particleList)):
                    self.particleList[count] = self.getParticleWithGhostInJail(self.particleList[count], i) #set particle to jail position and return it
                    count += 1 #increment loop
                
    def handleCaseOneJP(self, noisyDistance, particle, i):
        # #if noisyDistance is none then update all beliefs to 0 and provide the jail position in the dictionary
        if (noisyDistance == None):
            print " HELP -> "
            count = 0
            #self.particleList.append(self.getParticleWithGhostInJail(particle, i)) #set jail position in list of positions
            #update particle?
            particle = self.getParticleWithGhostInJail(particle, i) #set particle to jail position and return it
            print "my particle: ", particle
            #self.particleList = []; #set particle list to be empty and then fill it up with jail position
            tmp = [];
            while (count < len(self.posList)):
                #update particles with jail position?
                tmp.append(particle)
                count += 1 #increment loop
            self.particleList = tmp
            #print "particleList: ", self.legalPositions

    def handleCaseTwoJP(self, currentBeliefs, gameState, noisyDistances):
        #local declarations
        count = 0
        count2 = 0
        #if state to handle case with weights as 0
        if all(weight == 0.0 for weight in currentBeliefs.values()):
            self.initializeParticles()
            #self.checkNoisy(noisyDistances)
        else:
            count = 0
            tempList = []
            #loop to go through each particle
    ...
    def checkNoisy(self, noisyDistances):
        #local Declarations
        count = 0
        count2 = 0
        while (count < len(self.particleList)): #loop 1
            particle = self.particleList[count]
            while (count2 < self.numGhosts): #loop 2
                noisy = noisyDistances[count2]
                if (noisy == None):
                    particle = self.getParticleWithGhostInJail(particle, count2)
                count2 += 1 #increment inner loop
            count += 1 #increment loop
        count2 += 0 #reset inner loop
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particleList:
            ...
            "*** YOUR CODE HERE ***"
            #loop through each particle, each particle should be a tuple of positions for ghosts
            for position in newParticle:
                newGPositions = tuple()
                #loop through ghosts in range and calculate samples
                for i in range(self.numGhosts):
                    newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, prevGhostPositions), i, self.ghostAgents[i])
                    newSample = util.sample(newPosDist) #get new samples
                    newParticle[i] = newSample #update the particles with new sample
            "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        #loop through all legal positions for particles, and use pos as reference
        #while (count < len(self.posList)):
        #    positionRef = self.posList[count]
        #Loop through each particle with position reference, get sum of all particles by position perspective and put into dictionary
        while (count2 < len(self.particleList)):
            particle = self.particleList[count2] #get particle from list, particle is  tuple of ((x,y), weight)
            #only add sums if position Reference matches the position of the particle
            #if (particle == positionRef):
            #        sumVal += 1.0
            beliefList[particle] += 1 #beliefList[key=pos] = value=summation of OCCURRANCES
            count2 += 1 #increment counter2
        beliefList.normalize()
        return beliefList
