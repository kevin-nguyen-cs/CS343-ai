...
def enhancedFeatureExtractorDigit(datum):
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            # Add up white pixels per quadrant
            if datum.getPixel(x,y) > 0 and x < (DIGIT_DATUM_WIDTH/2) and y < (DIGIT_DATUM_HEIGHT/2):
                topLeft += 1
            if datum.getPixel(x,y) > 0 and x >= (DIGIT_DATUM_WIDTH/2) and y < (DIGIT_DATUM_HEIGHT/2):
                topRight += 1
            if datum.getPixel(x,y) > 0 and x < (DIGIT_DATUM_WIDTH/2) and y >= (DIGIT_DATUM_HEIGHT/2):
            ...
            # Look for cycles in the numbers
            if datum.getPixel(x,y) == 0:
                north = 0
                south = 0
                east = 0
                ...
                val1 = x
                val2 = y
                # check if there is a white pixel east
                while val1 > 0 and  val1 < DIGIT_DATUM_WIDTH:
                    if datum.getPixel(val1, y) > 0:
                        east = 1
                        break
                    val1 += 1
                # check if there is a white pixel west
                if east == 1:
                    val1 = x
                    val2 = y 
                    while val1 > 0 and  val1 < DIGIT_DATUM_WIDTH:
                        if datum.getPixel(val1, y) > 0:
                            ...
                            break
                        val1 -= 1
                # check if there is a white pixel north
                if west == 1:
                    val1 = x
                    val2 = y 
                    while val2 > 0 and  val2 < DIGIT_DATUM_HEIGHT:
                        if datum.getPixel(x, val2) > 0:
                            north = 1
                            break
                        val2 += 1
                # check if there is a white pixel south
                if north == 1:
                    val1 = x
                    val2 = y
                    while val2 > 0 and  val2 < DIGIT_DATUM_HEIGHT:
                ...
                x1 = str(x)
                y1 = str(y)
                cluster = 'cluster' + x1 + y1
                #if all conditions pass, mark black pixel as part of a cycle, otherwise, don't
                if (north == 1 and south == 1 and west == 1 and east == 1):
                    features[cluster] = 1
                else:
                    features[cluster] = 0

    # Find which quadrant has the most white pixels
    quadrants = [topLeft, topRight, bottomLeft, bottomRight]
    maxVal = max(quadrants)

    if topLeft == maxVal:
        features['topLeft'] = 1
        features['topRight'] = 0
        features['bottomLeft'] = 0
        features['bottomRight'] = 0
    elif topRight == maxVal:
        features['topLeft'] = 0
        features['topRight'] = 1
        features['bottomLeft'] = 0
        features['bottomRight'] = 0
    elif bottomLeft == maxVal:
        features['topLeft'] = 0
        features['topRight'] = 0
        features['bottomLeft'] = 1
        features['bottomRight'] = 0
    elif bottomRight == maxVal:
        features['topLeft'] = 0
        features['topRight'] = 0
        features['bottomLeft'] = 0
        features['bottomRight'] = 1
...
def enhancedPacmanFeatures(state, action):
    ...
    #FIND OUT WHETHER THE GHOST IS SCARED TO ASSIGN VALUES
    #IF THEY ARE SCARED, GIVE MINDIST TO GHOST = MAX DIST TO GHOST
    #MAYBE MAKE TOTAL DIST TO GHOST VERY HIGH
    #maybe have feature saying boolean scared values
    #Consider making larger values good
    #Maybe start from scratch T_T
    ...
    minGhostDist = 0
    ghostDistanceList = list()
    # Populate list with all manhattan distances to ghosts
    for ghost in ghostPositions:
        ghostDistanceList.append(util.manhattanDistance(pacman, ghost))
    
    minGhostDist = min(ghostDistanceList)

    #Avoid division by zero, maximize feature value base on proximity
    if minGhostDist > 0:
        minGhostDist = 100.0/minGhostDist
    features['ghostDist'] = minGhostDist

  
    # Get food positions in accessible manner
    foodPositions = successor.getFood().asList()
    minFoodDist = 0
    foodDistanceList = list()
    # Populate list with all manhattan distances between ghosts and pacman
    for food in foodPositions:
        foodDistanceList.append(util.manhattanDistance(pacman, food))
    
    # Avoid issue where foodDistanceList was empty
    if foodDistanceList:
        minFoodDist = min(foodDistanceList)

    # Avoid division by zero, maximize feature value base on proximity
    if minFoodDist > 0:
        minFoodDist = 100.0/minFoodDist
